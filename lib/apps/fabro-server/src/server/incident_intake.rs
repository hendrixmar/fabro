...
/// Secrets are read through the existing vault, never retained in settings or
/// errors.
pub(super) async fn validate_enablement(state: &super::AppState) -> anyhow::Result<()> {
    ...
    let bugsink = &settings.server.integrations.bugsink;
        ...
        !bugsink.dispatch_enabled || bugsink.enabled,
        "Bugsink dispatch requires enabled intake"
    );
    if !bugsink.enabled {
        return Ok(());
    }
    ...
    let url = url::Url::parse(origin).context("Bugsink origin is invalid")?;
        ...
        "Bugsink requires an exact HTTP(S) origin"
    );
    ensure!(
        !bugsink.projects.is_empty(),
        "Bugsink requires project mappings"
    );
    let token_name = bugsink
        .api_token_secret
        .as_deref()
        .context("Bugsink requires an API token vault name")?;
    let mut names = std::collections::HashSet::new();
    let mut projects = std::collections::HashSet::new();
    for name in std::iter::once(token_name).chain(
        bugsink
            .projects
            .iter()
            .map(|project| project.signing_secret.as_str()),
    ) {
        ensure!(
            fabro_types::is_env_style_name(name) && names.insert(name),
            "Bugsink requires unique valid vault token names"
        );
        let secret = state
            .vault_secret(name)
            .await
            .map_err(|_| anyhow::anyhow!("Bugsink vault lookup failed"))?;
        ensure!(
            secret
                .as_deref()
                .is_some_and(|value| !value.trim().is_empty()),
            "Bugsink required vault secret is missing or empty"
        );
    }
    for project in &bugsink.projects {
        ensure!(
            i64::try_from(project.project_id).is_ok() && projects.insert(project.project_id),
        ...
        let automation = state
            ...
            [25, 26].contains(&project.project_id),
        ...
        projects == std::collections::HashSet::from([25, 26]),
        "Bugsink requires both project mappings 25 and 26"
    );
    Ok(())
}
...
mod tests {
    ...
    fn enabled_intake_cannot_start_without_vault_keys_or_real_automation() {
        for with_keys in [false, true] {
            ...
            assert!(error.contains(if with_keys {
                "automation does not exist"
            } else {
                "vault secret is missing or empty"
            }));
