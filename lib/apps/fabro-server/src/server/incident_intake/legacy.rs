...
use super::{client::{bounded_json, http_client}, worker_store::{digest, incident_key}};
...
pub(super) async fn private_json(path: &Path) -> anyhow::Result<Value> {
    ...
    let metadata = tokio::fs::symlink_metadata(path).await?;
        ...
        && metadata.permissions().mode() & 0o077 == 0 && metadata.len() <= 1_048_576, "legacy_source_not_private");
    use tokio::io::AsyncReadExt;
    let mut bytes = Vec::new();
    tokio::fs::File::open(path).await?.take(1_048_577).read_to_end(&mut bytes).await?;
...
fn canonical_incident(value: &str) -> anyhow::Result<(u64, Uuid)> {
    ...
    ensure!(parts.len() == 3 && parts[0] == "bugsink", "legacy_incident_invalid");
    let project = parts[1].parse::<u64>()?;
    let issue = parts[2].parse::<Uuid>()?;
    ensure!(value == format!("bugsink:{project}:{issue}"), "legacy_incident_invalid");
    Ok((project, issue))
}

/// The old publisher emitted a plain paragraph, not an arbitrary occurrence of an ID.
/// Parse tag boundaries and accept only the complete marked text of that paragraph.
        ...
        if tag == "script" || tag == "style" {
            ...
            if rest[end..].starts_with("</p>") {
                if let Some(incident) = rest[..end].strip_prefix("incident: ") {
                    ...
                    if [25,26].contains(&project) { result.push(incident.to_owned()); }
                }
            }
        }
    }
...
fn origin(value: &str) -> anyhow::Result<url::Url> {
    ...
    ensure!(matches!(url.scheme(), "http" | "https") && url.origin().ascii_serialization() == value
        && url.username().is_empty() && url.password().is_none(), "legacy_origin_invalid");
    Ok(url)
}

pub(super) async fn import(state: &AppState) -> anyhow::Result<Value> {
    let path = state.active_config_path().with_file_name("bugsink-legacy.json");
    let config: LegacyConfig = serde_json::from_value(private_json(&path).await?)?;
    ensure!(config.schema_version == 1 && config.owners.keys().map(String::as_str).collect::<BTreeSet<_>>() == BTreeSet::from(["el-telar","laptop"]), "legacy_owners_missing");
    let http = http_client()?;
    let settings = state.server_settings();
    let bugsink_origin = settings.server.integrations.bugsink.origin.as_deref().context("bugsink_not_configured")?;
    ...
    for (name, owner) in &config.owners {
        ...
        let token = state.vault_secret(&owner.token_secret).await?.filter(|s| !s.trim().is_empty()).context("legacy_owner_credential_missing")?;
        let get = |path: String| {
            let http = &http; let token = &token; let api = &owner.api_origin;
            async move { bounded_json(http.get(format!("{api}/api/v1{path}")).bearer_auth(token).send().await?).await }
        };
        let inventory = get("/automations".into()).await?;
        let automation_rows = inventory.get("data").and_then(Value::as_array).context("legacy_inventory_invalid")?;
        let mut owned = Vec::new();
        for automation in automation_rows {
            let id = automation.get("id").and_then(Value::as_str).context("legacy_inventory_invalid")?;
            let workflow = automation.pointer("/target/workflow").and_then(Value::as_str).context("legacy_inventory_invalid")?;
            if ["bugsink-loop","incident-loop","ticket-loop"].contains(&id) || ["bugsink-loop","incident-loop","ticket-loop"].contains(&workflow) {
                ensure!(automation.get("triggers").and_then(Value::as_array).context("legacy_inventory_invalid")?
                    .iter().all(|trigger| trigger.get("enabled").and_then(Value::as_bool) == Some(false)), "legacy_owner_still_enabled");
        ...
        loop {
            ...
            let page = get(format!("/runs?page[limit]=100&page[offset]={offset}&include_archived=true&direction=asc")).await?;
            let rows = page.get("data").and_then(Value::as_array).context("legacy_runs_invalid")?;
            let more = page.pointer("/meta/has_more").and_then(Value::as_bool).context("legacy_runs_invalid")?;
            ensure!(!more || !rows.is_empty(), "legacy_runs_no_progress");
            for row in rows {
                let run_id = row.get("id").and_then(Value::as_str).context("legacy_run_id_invalid")?.parse::<RunId>()?;
                ...
                ensure!(projection.spec.run_id == run_id, "legacy_run_identity_mismatch");
                let incident = projection.spec.settings.run.inputs.get("incident").and_then(toml::Value::as_str);
                let owned_run = incident.is_some() || projection.spec.workflow_slug.as_deref().is_some_and(|slug|
                    ["bugsink-loop","incident-loop","ticket-loop"].contains(&slug)) || projection.spec.automation.as_ref().is_some_and(|a|
                    ["bugsink-loop","incident-loop","ticket-loop"].contains(&a.id.as_str()));
                ensure!(!owned_run || projection.status.is_terminal(), "legacy_run_in_flight");
                if let Some(incident) = incident {
                    let (project, _) = canonical_incident(incident)?;
                    if [25,26].contains(&project) { runs.entry(incident.to_owned()).or_default().push((run_id,projection)); }
                }
            }
            offset += rows.len();
            if !more { break; }
        ...
        for (incident, record) in local {
            let (project,_) = canonical_incident(incident)?;
            if ![25,26].contains(&project) { continue; }
            let status = record.get("status").and_then(Value::as_str).context("legacy_record_invalid")?;
    ...
    let api_base = plane.api_base.as_deref().context("legacy_plane_not_configured")?;
    let mut base = url::Url::parse(api_base)?;
    ensure!(matches!(base.scheme(), "http" | "https") && base.username().is_empty() && base.password().is_none() && base.query().is_none() && base.fragment().is_none(), "legacy_plane_origin_invalid");
    let workspace = plane.workspace.as_deref().context("legacy_plane_not_configured")?;
    ensure!(!workspace.is_empty() && workspace.bytes().all(|c| c.is_ascii_alphanumeric() || b"_-".contains(&c)), "legacy_plane_workspace_invalid");
    let path = format!("{}/workspaces/{workspace}/projects/{}/issues/", base.path().trim_end_matches('/'), config.plane_project_id);
    base.set_path(&path);
    let token = state.vault_secret(fabro_static::EnvVars::PLANE_API_KEY).await?.context("legacy_plane_credential_missing")?;
    ...
    loop {
        ...
        for ticket in page.get("results").and_then(Value::as_array).context("legacy_plane_page_invalid")? {
            let ticket_id = ticket.get("id").and_then(Value::as_str).context("legacy_ticket_invalid")?.parse::<Uuid>()?;
            ensure!(ticket.get("project").and_then(Value::as_str) == Some(config.plane_project_id.to_string().as_str()), "legacy_ticket_project_mismatch");
            for incident in html_incidents(ticket.get("description_html").and_then(Value::as_str).unwrap_or_default())? {
                tickets.entry(incident).or_default().insert(ticket_id.to_string());
            }
        }
        let more = page.get("next_page_results").and_then(Value::as_bool).context("legacy_plane_page_invalid")?;
        if !more { break; }
        let next = page.get("next_cursor").and_then(Value::as_str).context("legacy_plane_cursor_invalid")?;
        ensure!(!next.is_empty() && next.len() <= 1024 && seen.len() < 100_000 && seen.insert(next.to_owned()), "legacy_plane_cursor_repeated");
        cursor=Some(next.to_owned());
    }
    // Re-read the source inventories after the scan: a re-enabled owner invalidates ownership transfer.
    for (name,owner) in &config.owners {
        let token=state.vault_secret(&owner.token_secret).await?.context("legacy_owner_credential_missing")?;
        let inventory=bounded_json(http.get(format!("{}/api/v1/automations",owner.api_origin)).bearer_auth(token).send().await?).await?;
        let mut owned: Vec<_>=inventory.get("data").and_then(Value::as_array).context("legacy_inventory_invalid")?.iter()
            .filter(|a| a.get("id").and_then(Value::as_str).is_some_and(|id| ["bugsink-loop","incident-loop","ticket-loop"].contains(&id))
                || a.pointer("/target/workflow").and_then(Value::as_str).is_some_and(|id| ["bugsink-loop","incident-loop","ticket-loop"].contains(&id))).cloned().collect();
        owned.sort_by_key(|v|v["id"].as_str().unwrap_or_default().to_owned());
        ensure!(inventories.get(name) == Some(&digest(&owned)?), "legacy_inventory_changed");
    }
    ...
    for (incident,id,projection) in imported.values() {
        ...
        let revision=match projection.spec.git.as_ref().and_then(|git|git.sha.as_deref()).filter(|sha| super::worker::pinned_revision(sha)) {
