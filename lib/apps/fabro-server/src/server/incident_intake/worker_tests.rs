...
fn repeated_events_are_not_new_work() {
    assert!(!discovery_eligible(true, Some((false, false)), (false, false), 10, 0));
    assert!(discovery_eligible(true, Some((true, false)), (false, false), 10, 0));
    assert!(!discovery_eligible(true, Some((true, false)), (true, false), 10, 0));
...
fn cursor_cannot_change_coverage_or_repeat_after_restart() {
    ...
    progress.advance("https://bugsink.example", 25, Some(next)).unwrap();
    let mut resumed: ScanProgress = serde_json::from_str(&serde_json::to_string(&progress).unwrap()).unwrap();
    assert!(resumed.advance("https://bugsink.example", 25, Some(next)).is_err());
    ...
    assert!(resumed.advance("https://bugsink.example", 26, Some(next)).is_err());
}
...
fn build(dir: &std::path::Path, bundle: &(Arc<fabro_store::Database>,fabro_store::ArtifactStore),
    origin: &str, enabled: bool, dispatch: bool) -> Arc<AppState> {
...
async fn ready_observation(state: &AppState) -> (String,uuid::Uuid) {
    ...
    sqlx::query("UPDATE bugsink_scans SET baseline_complete=1").execute(&store.pool).await.unwrap();
    let alert=AcceptedAlert {project_id:25,issue_id:issue,reason:AlertReason::New,body_digest:"a".repeat(64),received_ms:10};
    store.accept(ORIGIN,&alert).await.unwrap();
    let pending=store.pending(ORIGIN, 10).await.unwrap().remove(0);
    store.apply_observation(ORIGIN,&pending,&client::Issue {project:25,id:issue,resolved:false,muted:false,first_seen_ms:10},event).await.unwrap();
    (pending.key,event)
}
...
async fn real_materialized_run_contains_only_trusted_five_inputs() {
    ...
    let id=state.incident_store().reserve(ORIGIN, 25, REVISION).await.unwrap().unwrap();
    ...
    for (key,value) in intent.inputs() {
        assert_eq!(projection.spec.settings.run.inputs.get(&key).and_then(toml::Value::as_str),Some(value.as_str()));
    }
    ...
    let checkpoint=fabro_workflow::event::Event::CheckpointCompleted {
        node_id:"investigate".into(),current_node:"investigate".into(),status:"succeeded".into(),
        completed_nodes:vec!["investigate".into()],node_retries:Default::default(),
        context_values:std::collections::BTreeMap::from([("incident_result".into(),report.clone())]),
        node_outcomes:std::collections::BTreeMap::from([("investigate".into(),fabro_types::Outcome::success())]),
        next_node_id:Some("exit".into()),git_commit_sha:None,loop_failure_signatures:Default::default(),
        restart_failure_signatures:Default::default(),node_visits:Default::default(),diff:None,diff_summary:None,
        graph_visit:Some(1),resumed_from_stage_id:None,
    };
    fabro_workflow::event::append_event(&run,&id,&checkpoint).await.unwrap();
    fabro_workflow::event::append_event(&run,&id,&fabro_workflow::event::Event::WorkflowRunCompleted {
        timing:fabro_types::RunTiming::wall_only(1),artifact_count:0,status:"succeeded".into(),reason:fabro_types::SuccessReason::Completed,
        total_usd_micros:None,final_git_commit_sha:None,final_patch:None,diff_summary:None,billing:None,
    }).await.unwrap();
    reconcile_run(&state,id).await.unwrap();
    assert_eq!(state.incident_store().intent(id).await.unwrap().state,"succeeded");
    let mut projection=run.state().await.unwrap();
    let original=projection.clone();
    projection.checkpoints.last_mut().unwrap().checkpoint.node_outcomes.remove("investigate");
    assert!(worker::terminal_result(&intent,&projection).is_err());
    projection=original.clone();
    report["publication"]=json!({"status":"completed"});
    projection.checkpoints.last_mut().unwrap().checkpoint.context_values.insert("incident_result".into(),report.clone());
    assert!(worker::terminal_result(&intent,&projection).is_err());
    projection=original;
    report["publication"]=json!({"status":"no_ticket"});
    report["run_id"]=json!(RunId::new().to_string());
    projection.checkpoints.last_mut().unwrap().checkpoint.context_values.insert("incident_result".into(),report);
    assert!(worker::terminal_result(&intent,&projection).is_err());
}
...
async fn submitted_run_resumes_same_identity_after_reconstruction() {
    ...
    let id=initial.incident_store().reserve(ORIGIN, 25, REVISION).await.unwrap().unwrap();
    let intent=initial.incident_store().intent(id).await.unwrap();
    let mut manifest=manifest();
    manifest.args.as_mut().unwrap().input=intent.inputs().into_iter()
        .map(|(key,value)|format!("{key}={}",serde_json::to_string(&value).unwrap())).collect();
    let bytes=serde_json::to_vec(&manifest).unwrap();
    assert!(initial.incident_store().begin_create(&intent,&bytes).await.unwrap());
    let response=Box::pin(super::super::handler::runs::create_run_from_manifest(Arc::clone(&initial),
        super::super::handler::runs::CreateRunFromManifestRequest {
            manifest,submitted_manifest_bytes:bytes,explicit_run_id:Some(id),explicit_title_supplied:true,
            actor:Principal::System {system_kind:SystemActorKind::Engine},headers:axum::http::HeaderMap::new(),
            automation:Some(AutomationRef {id:"incident-loop".into(),name:None,trigger_id:None}),
        })).await;
    assert_eq!(response.status(),axum::http::StatusCode::CREATED);
    assert_eq!(initial.stores.runs.open_run(&id).await.unwrap().state().await.unwrap().status,RunStatus::Submitted);
    drop(initial);
    let resumed=build(dir.path(),&bundle,ORIGIN,false,false);
    reconcile_run(&resumed,id).await.unwrap();
    assert_eq!(resumed.stores.runs.open_run(&id).await.unwrap().state().await.unwrap().status,RunStatus::Runnable);
    assert_eq!(resumed.incident_store().active_runs().await.unwrap(),vec![id]);
    assert!(resumed.incident_store().reserve(ORIGIN,26,REVISION).await.unwrap().is_none());
}
...
async fn created_only_run_and_missing_create_response_never_recreate() {
    ...
    let id=state.incident_store().reserve(ORIGIN, 25, REVISION).await.unwrap().unwrap();
    let intent=state.incident_store().intent(id).await.unwrap();
    state.incident_store().begin_create(&intent,b"attempted manifest").await.unwrap();
    // Even an authoritative missing Run cannot authorize recreating an attempted create.
    reconcile_run(&state,id).await.unwrap();
    assert_eq!(state.incident_store().intent(id).await.unwrap().state,"uncertain");
    ...
    fabro_workflow::event::append_event(&run,&id,&fabro_workflow::event::Event::RunCreated {
        run_id:id,title:None,settings:serde_json::to_value(fabro_types::WorkflowSettings::default()).unwrap(),
        graph:serde_json::to_value(fabro_types::Graph::new("partial")).unwrap(),workflow_source:None,
        labels:std::collections::BTreeMap::new(),source_directory:None,workflow_slug:None,automation:None,
        provenance:fabro_types::test_support::test_run_provenance(),manifest_blob:None,git:None,fork_source_ref:None,
        retried_from:None,parent_id:None,web_url:None,
    }).await.unwrap();
    drop(state);
    let resumed=build(dir.path(),&bundle,ORIGIN,false,false);
    reconcile_run(&resumed,id).await.unwrap();
    ...
    assert!(resumed.incident_store().reserve(ORIGIN,25,REVISION).await.unwrap().is_none());
}
...
async fn captured_generation_survives_restart_and_poll_never_resets_parked_budget() {
    ...
    let bundle=test_store_bundle();
    let state=build(dir.path(),&bundle,ORIGIN,false,false);
    let alert=AcceptedAlert {project_id:25,issue_id:uuid::Uuid::new_v4(),reason:AlertReason::New,body_digest:"a".repeat(64),received_ms:10};
    state.incident_store().accept(ORIGIN,&alert).await.unwrap();
    let first=state.incident_store().pending(ORIGIN, 10).await.unwrap().remove(0);
    drop(state);
    let resumed=build(dir.path(),&bundle,ORIGIN,false,false);
    let pending=resumed.incident_store().pending(ORIGIN, 10).await.unwrap().remove(0);
    assert_eq!(pending.generation,first.generation);
    let mut now=10;
    for attempt in 1..=5 {
        let pending=resumed.incident_store().pending(ORIGIN, now).await.unwrap().remove(0);
        resumed.incident_store().read_failed(&pending,now,"authoritative_read_failed").await.unwrap();
        if let Some(next)=retry_deadline(attempt,now) { now=next; }
    }
    let mut newer=alert.clone();newer.body_digest="b".repeat(64);
    resumed.incident_store().accept(ORIGIN,&newer).await.unwrap();
    resumed.incident_store().scan_issue(ORIGIN,&client::Issue {project:25,id:alert.issue_id,resolved:false,muted:false,first_seen_ms:10},true,0).await.unwrap();
    assert!(resumed.incident_store().pending(ORIGIN,now+1_000_000).await.unwrap().is_empty());
...
fn legacy_marker_is_whole_paragraph_not_a_substring_or_comment() {
    ...
    assert_eq!(legacy::html_incidents(&format!("<p>incident: {key}</p>")).unwrap(),vec![key]);
    assert!(legacy::html_incidents(&format!("<!--<p>incident: {key}</p>--><p>related {key}</p><p>incident: {key} extra</p>")).is_err());
    assert!(legacy::html_incidents(&format!("<!--<p>incident: {key}</p>--><p>related {key}</p>")).unwrap().is_empty());
}
...
async fn extra_run_is_explicit_audited_and_cannot_bypass_uncertainty() {
    ...
    store.accept(ORIGIN,&AcceptedAlert {project_id:25,issue_id:first.issue,reason:AlertReason::Regressed,body_digest:"b".repeat(64),received_ms:20}).await.unwrap();
    store.transition_run(&first,"failed",Some("investigation_failed")).await.unwrap();
    ...
    store.transition_run(&second,"failed",Some("investigation_failed")).await.unwrap();
    assert!(store.reserve(ORIGIN,25,REVISION).await.unwrap().is_none());
    let issue=client::Issue {project:25,id:first.issue,resolved:false,muted:false,first_seen_ms:10};
    store.operator_retry(ORIGIN,&issue,event,"operator",false,REVISION,1,30).await.unwrap();
    assert!(store.reserve(ORIGIN,25,REVISION).await.unwrap().is_none());
    store.operator_retry(ORIGIN,&issue,event,"operator",true,REVISION,1,31).await.unwrap();
    let third=store.active_runs().await.unwrap()[0];
    let row:(i64,Option<String>)=sqlx::query_as("SELECT attempt,authorized_by FROM bugsink_runs WHERE run_id=?")
        .bind(third.to_string()).fetch_one(&store.pool).await.unwrap();
    assert_eq!(row,(3,Some("operator".into())));
    assert_eq!(store.snapshot().await.unwrap()["incidents"][0]["requested_generation"],2);
    assert_eq!(store.snapshot().await.unwrap()["incidents"][0]["applied_generation"],1);
    let intent=store.intent(third).await.unwrap();
    store.transition_run(&intent,"uncertain",Some("create_response_uncertain")).await.unwrap();
    assert!(store.operator_retry(ORIGIN,&issue,event,"operator",true,REVISION,2,32).await.is_err());
    let raw:String=sqlx::query_scalar("SELECT cursor FROM bugsink_scans WHERE project_id=25").fetch_one(&store.pool).await.unwrap();
    let audit:ScanProgress=serde_json::from_str(&raw).unwrap();
    assert_eq!(audit.operator_audit.last().unwrap()["operator"],"operator");
    assert_eq!(store.snapshot().await.unwrap()["incidents"][0]["incident_key"],key);
}
...
async fn baseline_import_reads_actual_sources_and_exposes_only_verified_summary() {
    ...
    remote.mock(|when,then| {when.method(httpmock::Method::GET).path("/api/v1/runs");then.json_body(json!({"data":[],"meta":{"has_more":false,"total":0}}));});
...
async fn newer_reason_survives_a_captured_read_and_waiting_work_is_coalesced() {
    ...
    let issue=snapshot["incidents"][0]["issue_id"].as_str().unwrap().parse::<uuid::Uuid>().unwrap();
    let mut alert=AcceptedAlert {project_id:25,issue_id:issue,reason:AlertReason::New,body_digest:"b".repeat(64),received_ms:20};
    store.accept(ORIGIN,&alert).await.unwrap();
    assert!(store.pending("https://other.example",20).await.unwrap().is_empty());
    let captured=store.pending(ORIGIN,20).await.unwrap().remove(0);
    alert.reason=AlertReason::Regressed;alert.body_digest="c".repeat(64);
    store.accept(ORIGIN,&alert).await.unwrap();
    let current=client::Issue {project:25,id:issue,resolved:false,muted:false,first_seen_ms:10};
    store.apply_observation(ORIGIN,&captured,&current,uuid::Uuid::new_v4()).await.unwrap();
    let snapshot=store.snapshot().await.unwrap();
    ...
    let captured=store.pending(ORIGIN,20).await.unwrap().remove(0);
    let event=uuid::Uuid::new_v4();
    store.apply_observation(ORIGIN,&captured,&current,event).await.unwrap();
    assert!(store.reserve("https://other.example",25,REVISION).await.unwrap().is_none());
