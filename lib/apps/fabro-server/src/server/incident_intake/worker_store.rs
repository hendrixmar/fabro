...
use super::{IncidentStore, client::{Issue, ScanProgress}, discovery_eligible, retry_deadline};
...
impl Intent {
    pub fn inputs(&self) -> [(String, String); 5] {
        [("incident".into(), format!("bugsink:{}:{}", self.project, self.issue)),
         ("event".into(), self.event.to_string()), ("observation".into(), self.observation.clone()),
         ("episode".into(), self.episode.to_string()), ("run_id".into(), self.run_id.to_string())]
    }
}

impl IncidentStore {
    pub(super) async fn pending(&self, origin: &str, now: i64) -> anyhow::Result<Vec<PendingIncident>> {
        ...
        for row in rows {
            ...
            let generation = match captured { Some(value) => Some(value), None => self.capture_refresh(&key).await? };
            if let Some(generation) = generation {
                pending.push(PendingIncident { key, project: u64::try_from(row.try_get::<i64, _>("project_id")?)?,
                    issue: row.try_get::<String, _>("issue_id")?.parse()?, generation });
            }
        }
        Ok(pending)
    }

    pub(super) async fn read_failed(&self, pending: &PendingIncident, now: i64, reason: &str) -> anyhow::Result<()> {
        let mut tx = self.pool.begin().await?;
        let attempts: i64 = sqlx::query_scalar("SELECT read_attempts FROM bugsink_incidents WHERE incident_key=?").bind(&pending.key).fetch_one(&mut *tx).await?;
    ...
    pub(super) async fn apply_observation(&self, origin: &str, pending: &PendingIncident, issue: &Issue, event: Uuid) -> anyhow::Result<()> {
        let observation = digest(&(origin, issue.project, issue.id.to_string(), event.to_string(), issue.resolved, issue.muted))?;
        let mut tx = self.pool.begin().await?;
        ...
        let waiting = previous.is_some() && reason.is_some() && !old.try_get::<bool,_>("has_runs")?;
        let eligible = active && (waiting || (changed && (reason.as_deref().is_some_and(|r| matches!(r, "REGRESSED" | "UNMUTED"))
            || discovery_eligible(true, resolved.zip(muted), (issue.resolved, issue.muted), issue.first_seen_ms, boundary)
            || (previous.is_none() && reason.as_deref() == Some("NEW") && issue.first_seen_ms >= boundary))));
        let regression = eligible && changed && previous.is_some() && (resolved == Some(true) || reason.as_deref() == Some("REGRESSED"));
        // Keep the last eligible binding rather than turning ordinary repeated events into work.
        let observed_event = if eligible { Some(event.to_string()) } else { old.try_get::<Option<String>,_>("observed_event")? };
        let observation = if eligible { Some(observation) } else { previous };
    ...
    pub(super) async fn scan_issue(&self, origin: &str, issue: &Issue, complete: bool, boundary: i64) -> anyhow::Result<()> {
        ...
        let prior = old.as_ref().map(|row| Ok::<_, sqlx::Error>((row.try_get::<Option<bool>, _>("is_resolved")?, row.try_get::<Option<bool>, _>("is_muted")?)))
            .transpose()?.and_then(|(a,b)| a.zip(b));
        let eligible = discovery_eligible(complete, prior, (issue.resolved, issue.muted), issue.first_seen_ms, boundary);
        // Polling never clears read exhaustion or consumes an already-pending notification.
    ...
    pub(super) async fn reserve(&self, origin: &str, project: u64, revision: &str) -> anyhow::Result<Option<RunId>> {
        let mut tx = self.pool.begin().await?;
    ...
    pub(super) async fn intent(&self, run_id: RunId) -> anyhow::Result<Intent> {
        ...
        Ok(Intent { run_id, key:row.try_get("incident_key")?, project:u64::try_from(row.try_get::<i64,_>("project_id")?)?,
            issue:row.try_get::<String,_>("issue_id")?.parse()?, event:row.try_get::<String,_>("observed_event")?.parse()?,
            observation:row.try_get("observation_key")?, episode:row.try_get("episode")?, state:row.try_get("state")?,
            revision:row.try_get("source_revision")?, manifest_digest:row.try_get("manifest_digest")? })
    }
    ...
    pub(super) async fn active_runs(&self) -> anyhow::Result<Vec<RunId>> {
        ...
        ids.into_iter().map(|id| id.parse().map_err(Into::into)).collect()
    }

    pub(super) async fn transition_run(&self, intent: &Intent, state: &str, failure: Option<&str>) -> anyhow::Result<()> {
        let mut tx = self.pool.begin().await?;
        sqlx::query("UPDATE bugsink_runs SET state=?,failure_class=? WHERE run_id=?")
            .bind(state).bind(failure).bind(intent.run_id.to_string()).execute(&mut *tx).await?;
        if state == "failed" {
    ...
    pub(super) async fn start_baseline(&self, origin: &str, projects: &[u64], now: i64) -> anyhow::Result<()> {
        let mut tx = self.pool.begin().await?;
        ...
        for project in projects {
            sqlx::query("INSERT INTO bugsink_scans(origin,project_id,baseline_started_ms,cursor,scan_started_ms,next_scan_ms) VALUES(?,?,?,?,?,?)
                ...
                .execute(&mut *tx).await?;
            let raw: String = sqlx::query_scalar("SELECT cursor FROM bugsink_scans WHERE origin=? AND project_id=?")
                .bind(origin).bind(i64::try_from(*project)?).fetch_one(&mut *tx).await?;
    ...
    pub(super) async fn operator_retry(&self, origin: &str, issue: &Issue, event: Uuid, actor: &str,
        allow_additional_run: bool, revision: &str, generation: i64, now: i64) -> anyhow::Result<()> {
        ...
        ensure!(old.try_get::<Option<i64>,_>("refresh_generation")?.is_none()
            && old.try_get::<i64,_>("applied_generation")?<=generation && generation<=old.try_get::<i64,_>("requested_generation")?, "refresh_in_progress");
        let prior: Option<String>=old.try_get("observation_key")?;
        let changed=prior.as_deref()!=Some(observation.as_str());
        let episode=old.try_get::<i64,_>("episode")?+i64::from(changed && prior.is_some() && old.try_get::<Option<bool>,_>("is_resolved")?==Some(true));
        ...
        let succeeded: bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM bugsink_runs WHERE incident_key=? AND observation_key=? AND state='succeeded')")
            .bind(&key).bind(&observation).fetch_one(&mut *tx).await?;
        ensure!(!allow_additional_run || (!succeeded && !issue.resolved && !issue.muted), "observation_not_dispatchable");
        let row=sqlx::query("SELECT cursor,baseline_complete FROM bugsink_scans WHERE origin=? AND project_id=?")
            .bind(origin).bind(i64::try_from(issue.project)?).fetch_one(&mut *tx).await?;
        ...
        sqlx::query("UPDATE bugsink_scans SET cursor=? WHERE origin=? AND project_id=?")
            .bind(serde_json::to_string(&progress)?).bind(origin).bind(i64::try_from(issue.project)?).execute(&mut *tx).await?;
        tx.commit().await?;
        Ok(())
    }
}
