...
pub(super) fn retry_deadline(attempt: u8, now: i64) -> Option<i64> {
    [15_000, 30_000, 60_000, 120_000]
        .get(usize::from(attempt.checked_sub(1)?))
...
pub(crate) async fn begin_baseline(state: &Arc<AppState>, now: i64) -> anyhow::Result<()> {
    let config = state.server_settings().server.integrations.bugsink.clone();
        ...
        config.enabled && !config.dispatch_enabled,
        "baseline_requires_disabled_dispatch"
    );
    ...
    for project in &projects {
        mapped_automation(state, *project).await?;
    }
    state
        .incident_store()
        .start_baseline(
            config.origin.as_deref().context("bugsink_not_configured")?,
            &projects,
            now,
        )
        .await
...
pub(crate) async fn operator_retry(
    ...
    let config = state.server_settings().server.integrations.bugsink.clone();
        ...
        config.enabled && (!allow_additional_run || config.dispatch_enabled),
        "dispatch_disabled"
    );
    ...
    let generation: i64 = sqlx::query_scalar(
        "SELECT requested_generation FROM bugsink_incidents WHERE incident_key=?",
    )
    .bind(key)
    .fetch_one(&state.incident_store().pool)
    .await?;
...
pub(super) async fn mapped_automation(
    state: &AppState,
    project: u64,
    ...
    let mapping = config
        .projects
        .iter()
        .find(|mapping| mapping.project_id == project)
        .context("unmapped_project")?;
    let id = AutomationId::new(mapping.automation_id.clone())
        .map_err(|_| anyhow::anyhow!("invalid_automation"))?;
...
pub(crate) async fn reconcile_run(state: &Arc<AppState>, run_id: RunId) -> anyhow::Result<()> {
    ...
    if !matches!(
        intent.state.as_str(),
        "reserved" | "creating" | "submitted" | "active" | "uncertain"
    ) {
        return Ok(());
    }
    let run = match state.stores.runs.open_run(&run_id).await {
        ...
        Err(fabro_store::Error::RunNotFound(_))
            if intent.state == "reserved" && intent.manifest_digest.is_none() =>
        {
            // Disablement stops new creation too, including a reservation surviving a
            // restart.
            if !state
                .server_settings()
                .server
        ...
        Err(_) => {
            return store
                .transition_run(&intent, "uncertain", Some("run_storage_uncertain"))
                .await;
        }
    };
    let submitted = run
    ...
    match projection.status {
        RunStatus::Submitted => {
            ...
            super::super::handler::lifecycle::queue_run_start(
                state.as_ref(),
                run_id,
                false,
                engine(),
        ...
        RunStatus::Succeeded { .. } => {
            if terminal_result(&intent, &projection).is_ok() {
                ...
                store
                    .transition_run(&intent, "failed", Some("incident_handoff_incomplete"))
                    .await
            }
        }
        status if status.is_terminal() => {
            store
                .transition_run(&intent, "failed", Some("investigation_failed"))
                .await
        }
...
async fn materialize_create(state: &Arc<AppState>, intent: &Intent) -> anyhow::Result<()> {
    ...
    .await;
    // A response failure can follow RunCreated or RunSubmitted. Never recreate this
    // identity.
...
fn verify_binding(
    ...
    automation: &Automation,
        ...
        projection.spec.run_id == intent.run_id
            && projection
                .spec
                .automation
                .as_ref()
...
pub(super) fn terminal_result(intent: &Intent, projection: &RunProjection) -> anyhow::Result<()> {
        ...
        projection
            .conclusion
            .as_ref()
            .is_some_and(|conclusion| conclusion.status == StageOutcome::Succeeded),
        "conclusion_missing"
    );
    let checkpoint = projection
        .current_checkpoint()
        .context("checkpoint_missing")?;
    let required = projection
        .spec
        .graph
        .nodes
        .get("investigate")
        .context("required_stage_missing")?;
    ensure!(
        required.goal_gate() && required.output_schema() == Some("routing"),
        "required_stage_not_gated"
    );
        ...
        .filter(|node| node.goal_gate())
    {
            ...
            checkpoint
                .node_outcomes
                .get(&node.id)
                .is_some_and(|outcome| outcome.status == StageOutcome::Succeeded),
    ...
    let value = checkpoint
        .context_values
        .get("incident_result")
        .context("incident_result_missing")?;
    ensure!(
        serde_json::to_vec(value)?.len() <= 16_384,
        "incident_result_oversized"
    );
    ...
    let allowed = [
        ...
        (10..=11).contains(&object.len())
            && object.keys().all(|key| allowed.contains(&key.as_str()))
            && value.get("schema_version").and_then(Value::as_u64) == Some(1)
            && value.get("status").and_then(Value::as_str) == Some("completed"),
        "incident_result_invalid"
    );
    for (key, expected) in intent.inputs() {
        if key == "episode" {
            ensure!(
                value.get(&key).and_then(Value::as_i64) == Some(intent.episode),
                "incident_result_binding_mismatch"
            );
        } else {
            ensure!(
                value.get(&key).and_then(Value::as_str) == Some(expected.as_str()),
                "incident_result_binding_mismatch"
            );
        }
    }
    let digest = value
        ...
        .context("evidence_digest_missing")?;
    ensure!(
        digest.len() == 64
            && digest
                .bytes()
    ...
    let missing = value
        ...
        .context("missing_evidence_invalid")?;
    ensure!(
        missing.len() <= 64
            && missing
                .iter()
        ...
        Some("completed") => {
            ...
            for name in ["project_id", "issue_id"] {
                let text = publication
                    .get(name)
                    .and_then(Value::as_str)
                    .context("publication_identity_missing")?;
                ensure!(
                    text.parse::<uuid::Uuid>()?.to_string() == text,
    ...
    if let Some(children) = value.get("child_run_ids") {
        ...
        for child in children {
            let id = child
                .as_str()
                .context("child_id_invalid")?
                .parse::<RunId>()?;
            ensure!(id != intent.run_id, "child_id_invalid");
        }
    }
    Ok(())
}
...
pub(super) async fn scan_once(
    ...
    let store = state.incident_store();
    let rows = sqlx::query(
        "SELECT * FROM bugsink_scans WHERE origin=? AND baseline_started_ms IS NOT NULL
    ...
    for row in rows {
        if row
            .try_get::<Option<i64>, _>("next_scan_ms")?
        ...
        if progress.parked_reason.is_some() {
            continue;
        }
        ...
        let result: anyhow::Result<bool> = async {
            ...
            if row.try_get::<Option<i64>, _>("scan_started_ms")?.is_none() {
                ...
                progress.seen.clear();
                progress.seen_issues.clear();
            }
            let page = client.page(project, progress.cursor.as_deref()).await?;
            let values = page
                .get("results")
                .and_then(Value::as_array)
                .context("invalid_scan_page")?;
            let next = page.get("next").context("invalid_scan_page")?;
            ensure!(next.is_null() || next.is_string(), "invalid_scan_cursor");
            let issues: Vec<_> = values
                .iter()
                .map(|value| Issue::parse(value, project, None))
                .collect::<anyhow::Result<_>>()?;
            for issue in &issues {
                ensure!(
                    progress.seen_issues.len() < 100_000 && progress.seen_issues.insert(issue.id),
                    "repeated_scan_issue"
                );
            }
            ...
            for issue in issues {
                store
                    .scan_issue(&client.origin, &issue, complete, boundary)
                    .await?;
            }
            Ok(next.is_null())
        }
