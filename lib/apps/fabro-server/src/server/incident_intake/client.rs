...
impl Issue {
    pub(super) fn parse(value: &Value, project: u64, expected: Option<Uuid>) -> anyhow::Result<Self> {
        let id = value
            ...
            .context("issue_identity_missing")?
            .parse::<Uuid>()?;
        ensure!(
            value.get("project").and_then(Value::as_u64) == Some(project)
        ...
        let resolved = value
            ...
            .context("issue_state_missing")?;
        let muted = value
            .get("is_muted")
            .and_then(Value::as_bool)
            .context("issue_state_missing")?;
        let first_seen_ms = chrono::DateTime::parse_from_rfc3339(
            value
                .get("first_seen")
                .and_then(Value::as_str)
                .context("issue_timestamp_missing")?,
        )?
        .timestamp_millis();
        Ok(Self {
            project,
...
impl BugsinkClient {
    pub(super) async fn new(state: &AppState) -> anyhow::Result<Self> {
        ...
        let name = config
            ...
            fabro_types::is_env_style_name(name),
            "invalid_bugsink_token_reference"
        );
        let token = state
            .vault_secret(name)
            .await?
            .filter(|s| !s.trim().is_empty())
            .context("bugsink_credential_unavailable")?;
        Ok(Self {
            origin,
            http: http_client()?,
            token,
    ...
    pub(super) async fn issue(&self, project: u64, id: Uuid) -> anyhow::Result<(Issue, Uuid)> {
        ...
        url.query_pairs_mut()
            .append_pair("issue", &id.to_string())
        ...
        let event = page
            ...
            .context("event_not_found_or_retained")?;
        let event_id = event
            .get("id")
            .and_then(Value::as_str)
            .context("event_identity_missing")?
            .parse::<Uuid>()?;
        ensure!(
            event.get("project").and_then(Value::as_u64) == Some(project)
                && event.get("issue").and_then(Value::as_str) == Some(id.to_string().as_str()),
...
impl ScanProgress {
    pub(super) fn advance(
        ...
        for (key, expected) in [
            ...
            pairs.iter().all(|(key, _)| matches!(
                key.as_ref(),
                "project" | "sort" | "order" | "cursor" | "limit"
            )),
            "invalid_scan_query"
        );
        let cursors: Vec<_> = pairs
            .iter()
        ...
        let cursor = cursors[0];
            ...
            !cursor.is_empty()
                ...
                && !self.seen.iter().any(|old| old == cursor)
                && self.seen.len() < 100_000,
            "invalid_or_repeated_scan_cursor"
        );
