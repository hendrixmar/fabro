...
impl BlobStore {
    ...
    pub async fn read(&self, id: &BlobHash) -> Result<Option<Bytes>> {
        match &self.backend {
            BlobBackend::Slate(repo) => Ok(repo.get(id).await?.map(|blob| blob.0)),
            BlobBackend::Sqlite(pool) => {
                ...
                if BlobHash::new(&stored) != *id {
                    return Err(Error::BlobIntegrity { blob_hash: *id });
                }
    ...
    pub async fn exists(&self, id: &BlobHash) -> Result<bool> {
        match &self.backend {
            BlobBackend::Slate(repo) => repo.exists(id).await,
            BlobBackend::Sqlite(pool) => {
                let exists: bool =
