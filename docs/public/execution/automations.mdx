...
### Plane ticket triggers

A Plane trigger polls one configured Plane project for tickets in a Ready state, claims them, and starts runs through the same materialization path as API and schedule triggers. Lifecycle state IDs are exact Plane UUIDs. The trigger default harness is `codex` or `omp`; optional label IDs can override that default. Ticket text never becomes a command.

v1 retry policy is one automatic retry. After the second failure the ticket stays In Progress, Fabro posts a failure comment, and the optional failure label is applied. Completion requires a successful run plus a linked draft PR; Fabro does not wait for merge or CI.

v1 is single-server only. Do not run two Fabro servers against the same store for claiming.
